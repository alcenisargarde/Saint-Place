...
/obj/structure/globe
	...
	and on the northern half of the western continent, a modest peninsula marked as 'New Psydonia'."
	icon = 'icons/roguetown/misc/structure.dmi'
	icon_state = "globe"
	density = TRUE
	anchored = FALSE
...
/obj/structure/globe/attack_hand(mob/user)
	...
	var/mob/living/carbon/human/H = user
	var/random_message = pick("you spin the globe!", "You land on New Psydonia!", "You land on Raneshen!", "You land on port Ice cube!.", "You land on port Thornvale!", "You land on grenzelhoft!")
	to_chat(H, span_notice("[random_message]"))
