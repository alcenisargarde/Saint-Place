...
/datum/antagonist/suitor/greet()
	to_chat(owner.current, span_userdanger("I've been sent here with a purpose. I must secure a marriage with the local duke in order to bolster my house's relations with New Psydonia, by any means necessary."))
...
/datum/antagonist/suitor/roundend_report()
	...
	if(traitorwin)
		...
		to_chat(world, span_greentext("[owner.current.real_name] successfully married [marriagepartner] and secured an alliance with New Psydonia for their house!"))
		if(owner?.current)
			owner.current.playsound_local(get_turf(owner.current), 'sound/misc/triumph.ogg', 100, FALSE, pressure_affected = FALSE)
	else
		to_chat(owner.current, span_redtext("I've failed to secure a marriage into the throne of New Psydonia! My house will be disappointed!"))
